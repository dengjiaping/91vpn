...
import com.dmtec.a91jiasu.data.VpnProfile.SelectedAppsHandling;
import com.dmtec.a91jiasu.data.VpnProfileDataSource;
import com.dmtec.a91jiasu.data.VpnType.VpnTypeFeature;
import com.dmtec.a91jiasu.logic.VpnStateService.ErrorState;
import com.dmtec.a91jiasu.logic.VpnStateService.State;
import com.dmtec.a91jiasu.logic.imc.ImcState;
import com.dmtec.a91jiasu.logic.imc.RemediationInstruction;
import com.dmtec.a91jiasu.ui.MainActivity;
import com.dmtec.a91jiasu.ui.StrongSwanApplication;
import com.dmtec.a91jiasu.utils.IPRange;
import com.dmtec.a91jiasu.utils.IPRangeSet;
import com.dmtec.a91jiasu.utils.SettingsWriter;
...
public class CharonVpnService extends VpnService implements Runnable, VpnStateService.VpnStateListener
{
	...
	public static final String DISCONNECT_ACTION = "com.dmtec.a91jiasu.CharonVpnService.DISCONNECT";
	...
	private Notification buildNotification(boolean publicVersion)
	{
		...
		if (!publicVersion)
		{
			...
			builder.setContentText("91加速正在为您服务");
	...
	public void stateChanged() {
		...
		ErrorState errorState = mService.getErrorState();
		State state = mService.getState();
		Intent intent = new Intent("com.dmtec.a91jiasu.msg.receiver");
		if(errorState != ErrorState.NO_ERROR){
			intent.putExtra("state",-1000);
			((StrongSwanApplication)getApplication()).vpnPut("CONNECT_STATE",-1000);
			...
			switch (state)
			{
				case CONNECTING:
					...
					((StrongSwanApplication)getApplication()).vpnPut("CONNECT_STATE",1);
					sendBroadcast(intent);
                    Log.e("status:","连接中");
					break;
				case CONNECTED:
					intent.putExtra("state",2);
					((StrongSwanApplication)getApplication()).vpnPut("CONNECT_STATE",2);
                    sendBroadcast(intent);
                    Log.e("status:","连接成功");
					break;
				case DISCONNECTING:
					intent.putExtra("state",3);
					((StrongSwanApplication)getApplication()).vpnPut("CONNECT_STATE",3);
                    sendBroadcast(intent);
                    Log.e("status:","断开连接");
					break;
			}
	...
	/*
	 * The libraries are extracted to /data/data/com.dmtec.a91jiasu/...
